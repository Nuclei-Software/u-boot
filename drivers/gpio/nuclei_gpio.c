...
struct nuclei_gpio_plat {
	void *base;     /* address of registers in physical memory */
};

static int nuclei_gpio_probe(struct udevice *dev)
{
	struct nuclei_gpio_plat *plat = dev_get_plat(dev);
...
static int nuclei_gpio_ofdata_to_plat(struct udevice *dev)
{
	struct nuclei_gpio_plat *plat = dev_get_plat(dev);
...
U_BOOT_DRIVER(gpio_nuclei) = {
	...
	.of_to_plat = of_match_ptr(nuclei_gpio_ofdata_to_plat),
	.plat_auto`` = sizeof(struct nuclei_gpio_plat),
	.ops	= &nuclei_gpio_ops,
